...
const appState = {
  ...
  /* ── Chord Diagram mode ── */
  chordDiagramMode      : false,
  chordDiagramEditIndex : -1,
...
/* ═══════════════════════════════════════
   §1.5  CHORD DIAGRAM — Constants & Data
   ═══════════════════════════════════════ */

/* Panel canvas drawing constants */
const CDP = { CELL_W:14, CELL_H:16, PAD_L:22, PAD_R:8, PAD_TOP:28, PAD_BOT:8, NUT_H:4, DOT_R:5, FRETS:4 };

/* Panel editor runtime state */
const cdpState = {
  inst      : 'guitar',
  startFret : 1,
  frets     : [-1,-1,-1,-1,-1,-1],
...
/* Open-string semitones (C=0). Guitar: E A D G B E (str 6→1). Uke: G C E A (str 4→1). */
const GUITAR_OPEN = [4,9,2,7,11,4];
const UKE_OPEN    = [7,0,4,9];

/* Chord-interval patterns → suffix string ('' = major) */
const CHORD_INTERVALS = {
  '0,4,7':'','0,3,7':'m','0,4,7,10':'7','0,3,7,10':'m7',
  '0,4,7,11':'maj7','0,3,6':'dim','0,3,6,9':'dim7',
  '0,4,8':'aug','0,2,7':'sus2','0,5,7':'sus4',
  '0,4,7,9':'6','0,3,7,9':'m6','0,3,6,10':'m7b5',
  '0,2,4,7':'add9','0,2,4,7,10':'9','0,2,3,7,10':'m9',
};
const CDP_NOTE_NAMES = ['C','C#','D','Eb','E','F','F#','G','Ab','A','Bb','B'];

/* Guitar voicings: f=frets, g=fingers, b=barre, sf=startFret */
const GUITAR_VOICINGS = {
  'C'    :{f:[-1,3,2,0,1,0],g:[0,3,2,0,1,0]},
  'Cm'   :{f:[-1,3,5,5,4,3],g:[0,2,4,3,1,1],sf:3},
  ...
  'F'    :{f:[1,3,3,2,1,1],g:[1,3,4,2,1,1],b:{fret:1,from:0,to:5}},
  'Fm'   :{f:[1,3,3,1,1,1],g:[1,3,4,1,1,1],b:{fret:1,from:0,to:5}},
  'F7'   :{f:[1,3,1,2,1,1],g:[1,3,1,2,1,1],b:{fret:1,from:0,to:5}},
  'G'    :{f:[3,2,0,0,0,3],g:[2,1,0,0,0,3]},
  'Gm'   :{f:[3,5,5,3,3,3],g:[1,3,4,1,1,1],b:{fret:3,from:0,to:5}},
  'G7'   :{f:[3,2,0,0,0,1],g:[3,2,0,0,0,1]},
  'Gmaj7':{f:[3,2,0,0,0,2],g:[3,2,0,0,0,2]},
  'A'    :{f:[-1,0,2,2,2,0],g:[0,0,1,2,3,0]},
  'Am'   :{f:[-1,0,2,2,1,0],g:[0,0,2,3,1,0]},
  'A7'   :{f:[-1,0,2,0,2,0],g:[0,0,2,0,3,0]},
  'Am7'  :{f:[-1,0,2,0,1,0],g:[0,0,2,0,1,0]},
  'Amaj7':{f:[-1,0,2,1,2,0],g:[0,0,2,1,3,0]},
  'B7'   :{f:[-1,2,1,2,0,2],g:[0,2,1,3,0,4]},
  'Bm'   :{f:[-1,2,4,4,3,2],g:[0,1,3,4,2,1],b:{fret:2,from:1,to:5}},
};

/* Ukulele voicings (GCEA, str 4→1) */
const UKE_VOICINGS = {
  'C'    :{f:[0,0,0,3],g:[0,0,0,3]},
  'Cm'   :{f:[0,3,3,3],g:[0,1,2,3]},
  'C7'   :{f:[0,0,0,1],g:[0,0,0,1]},
  'Cmaj7':{f:[0,0,0,2],g:[0,0,0,2]},
  'D'    :{f:[2,2,2,0],g:[1,2,3,0]},
  'Dm'   :{f:[2,2,1,0],g:[2,3,1,0]},
  'D7'   :{f:[2,2,2,3],g:[1,2,3,4]},
  'E'    :{f:[4,4,4,2],g:[2,3,4,1],b:{fret:4,from:0,to:2}},
  'Em'   :{f:[0,4,3,2],g:[0,4,3,2]},
  'E7'   :{f:[1,2,0,2],g:[1,2,0,3]},
  'F'    :{f:[2,0,1,0],g:[2,0,1,0]},
  'Fm'   :{f:[1,0,1,3],g:[1,0,2,4]},
  'F7'   :{f:[2,3,1,3],g:[2,3,1,4]},
  'G'    :{f:[0,2,3,2],g:[0,1,3,2]},
  'Gm'   :{f:[0,2,3,1],g:[0,2,3,1]},
  'G7'   :{f:[0,2,1,2],g:[0,2,1,3]},
  'A'    :{f:[2,1,0,0],g:[2,1,0,0]},
  'Am'   :{f:[2,0,0,0],g:[2,0,0,0]},
  'A7'   :{f:[0,1,0,0],g:[0,1,0,0]},
  'B7'   :{f:[2,3,2,2],g:[1,3,2,4]},
  'Bm'   :{f:[4,2,2,2],g:[4,1,2,3]},
};

const GUITAR_PRESETS = ['C','Am','Em','G','G7','D','Dm','F','E','E7','A','A7','Bm','B7'];
const UKE_PRESETS    = ['C','Am','Em','G','G7','D','Dm','F','E7','A','Am','C7'];
...
const ARTICULATION_DEFS = {
  ...
  'down-strum'    : { label: 'Down Strum'      },
  'up-strum'      : { label: 'Up Strum'        },
};
...
function applyAccessLocks() {
  ...
  if (!hasFullToolsAccess()) {
    ...
    [
      ...
      document.getElementById('btn-chord-diagram'),
...
function renderScore() {
  ...
  if (svg2) {
    ...
    /* ── Draw chord diagrams above notes ── */
    const rowToStaveY = new Map();
    appState.staveCache.forEach(sc => rowToStaveY.set(sc.row, sc.y));

    appState.renderedBBoxes.forEach(bb => {
      if (bb.voice !== 0) return;
      const note = appState.notes[bb.globalIndex];
      if (!note || !note.chordDiagram) return;
      const sc = appState.staveCache.find(s =>
        bb.globalIndex >= s.startNoteIdx && bb.globalIndex <= s.endNoteIdx);
      const staveY  = sc ? sc.y : STAVE_Y_START;
      const xCenter = bb.x + bb.w / 2;
      const yBottom = staveY - 6;
      drawChordDiagramSVG(svg2, xCenter, yBottom, note.chordDiagram);
    });
...
function strumCenterY(bb, sls) {
  ...
  return staffTop - Math.max(13, sls * 1.35);
}

function drawDownStrum(svg, bb, sls) {
  const cx = bb.x + bb.w / 2;
  const cy = strumCenterY(bb, sls);
  const w = Math.max(8, sls * 0.9);
  const h = Math.max(11, sls * 1.15);
  const yTop = cy - h / 2;
  const yBot = cy + h / 2;
  svg.append(
    svgEl('line', { x1: cx - w / 2, y1: yTop, x2: cx + w / 2, y2: yTop, stroke: '#000', 'stroke-width': '1.7', 'stroke-linecap': 'round' }),
    svgEl('line', { x1: cx - w / 2, y1: yTop, x2: cx - w / 2, y2: yBot, stroke: '#000', 'stroke-width': '1.7', 'stroke-linecap': 'round' }),
    svgEl('line', { x1: cx + w / 2, y1: yTop, x2: cx + w / 2, y2: yBot, stroke: '#000', 'stroke-width': '1.7', 'stroke-linecap': 'round' })
  );
}

function drawUpStrum(svg, bb, sls) {
  const cx = bb.x + bb.w / 2;
  const cy = strumCenterY(bb, sls);
  const w = Math.max(9, sls * 1.0);
  const h = Math.max(11, sls * 1.15);
  const yTop = cy - h / 2;
  const yBot = cy + h / 2;
  svg.append(
    svgEl('line', { x1: cx - w / 2, y1: yTop, x2: cx, y2: yBot, stroke: '#000', 'stroke-width': '1.7', 'stroke-linecap': 'round' }),
    svgEl('line', { x1: cx, y1: yBot, x2: cx + w / 2, y2: yTop, stroke: '#000', 'stroke-width': '1.7', 'stroke-linecap': 'round' })
  );
}
...
function drawAllArticulations(svg) {
  ...
  appState.renderedBBoxes.forEach(bb => {
    ...
    switch (note.articulation) {
      ...
      case 'down-strum':    drawDownStrum(svg, bb, sls);          break;
      case 'up-strum':      drawUpStrum(svg, bb, sls);            break;
    }
  });
}
...
/* ═══════════════════════════════════════
   §9.7  CHORD DIAGRAM ENTRY (Guitar Pro style)
   ═══════════════════════════════════════
   Workflow:
     1. Click "Diagram" button → enter diagram mode
     2. Click a note → CDP panel opens for that note
     3. Click fretboard cells to place/remove fingers
     4. Drag across same fret → barre chord
     5. Click string indicator → toggle muted/open
     6. Use presets for common chords
     7. Click 삽입 → commit diagram to note
     8. ◀/▶ → navigate to prev/next note
     9. × or Escape → exit mode
   ═══════════════════════════════════════ */

/* ── Chord recognition: fret positions → chord name ── */
function cdpDetectChordName() {
  const open = cdpState.inst === 'guitar' ? GUITAR_OPEN : UKE_OPEN;
  const strCount = open.length;
  const pcs = new Set();
  for (let s = 0; s < strCount; s++) {
    const f = cdpState.frets[s];
    if (f < 0) continue;
    pcs.add((open[s] + f) % 12);
  }
  if (pcs.size === 0) return '—';
  if (pcs.size === 1) return CDP_NOTE_NAMES[[...pcs][0]];

  const arr = [...pcs].sort((a, b) => a - b);

  function tryRoot(root, notes) {
    const ivls = notes.map(n => (n - root + 12) % 12).sort((a, b) => a - b);
    const q = CHORD_INTERVALS[ivls.join(',')];
    if (q !== undefined) return CDP_NOTE_NAMES[root] + q;
    return null;
  }

  for (const root of arr) {
    const r = tryRoot(root, arr);
    if (r) return r;
  }
  if (arr.length > 3) {
    for (let skip = 0; skip < arr.length; skip++) {
      const sub = arr.filter((_, i) => i !== skip);
      for (const root of sub) {
        const r = tryRoot(root, sub);
        if (r) return r;
      }
    }
  }
...
function renderDiagramCanvas() {
  ...
  const strCount = cdpState.inst === 'guitar' ? 6 : 4;
  const W = CDP.PAD_L + (strCount - 1) * CDP.CELL_W + CDP.PAD_R;
  const H = CDP.PAD_TOP + CDP.NUT_H + CDP.FRETS * CDP.CELL_H + CDP.PAD_BOT;
  ...
  const sf = cdpState.startFret;

  /* Nut or fret position label */
  if (sf === 1) {
    ctx.fillStyle = '#2a2520';
    ctx.fillRect(CDP.PAD_L - 1, CDP.PAD_TOP, (strCount - 1) * CDP.CELL_W + 2, CDP.NUT_H);
  } else {
    ctx.strokeStyle = '#aaa';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(CDP.PAD_L, CDP.PAD_TOP);
    ctx.lineTo(CDP.PAD_L + (strCount - 1) * CDP.CELL_W, CDP.PAD_TOP);
    ctx.stroke();
    ctx.fillStyle = '#666';
    ctx.font = '10px sans-serif';
    ctx.textAlign = 'right';
    ctx.textBaseline = 'middle';
    ctx.fillText(sf + 'fr', CDP.PAD_L - 4, CDP.PAD_TOP + CDP.NUT_H + CDP.CELL_H * 0.5);
  }

  /* Fret lines */
  ctx.strokeStyle = '#ccc';
  ctx.lineWidth = 0.8;
  for (let f = 0; f <= CDP.FRETS; f++) {
    const y = CDP.PAD_TOP + CDP.NUT_H + f * CDP.CELL_H;
    ctx.beginPath();
    ctx.moveTo(CDP.PAD_L, y);
    ctx.lineTo(CDP.PAD_L + (strCount - 1) * CDP.CELL_W, y);
    ctx.stroke();
  }

  /* String lines */
  for (let s = 0; s < strCount; s++) {
    const x = CDP.PAD_L + s * CDP.CELL_W;
    ctx.strokeStyle = '#aaa';
    ctx.lineWidth = 0.8;
    ctx.beginPath();
    ctx.moveTo(x, CDP.PAD_TOP);
    ctx.lineTo(x, CDP.PAD_TOP + CDP.NUT_H + CDP.FRETS * CDP.CELL_H);
    ctx.stroke();
  }

  /* Barre bar */
  if (cdpState.barre) {
    const { fret, from, to } = cdpState.barre;
    const relF = fret - sf;
    if (relF >= 0 && relF < CDP.FRETS) {
      const barY = CDP.PAD_TOP + CDP.NUT_H + relF * CDP.CELL_H + CDP.CELL_H * 0.5;
      const x1   = CDP.PAD_L + from * CDP.CELL_W;
      const x2   = CDP.PAD_L + to   * CDP.CELL_W;
      ctx.fillStyle = '#2a2520';
      ctx.beginPath();
      ctx.arc(x1, barY, CDP.DOT_R, Math.PI * 0.5, Math.PI * 1.5);
      ctx.arc(x2, barY, CDP.DOT_R, -Math.PI * 0.5, Math.PI * 0.5);
      ctx.closePath();
      ctx.fill();
    }
  }

  /* Per-string: indicator + finger dot */
  const iy = CDP.PAD_TOP * 0.5;
  for (let s = 0; s < strCount; s++) {
    const f = cdpState.frets[s];
    const x = CDP.PAD_L + s * CDP.CELL_W;

    /* Indicator row (open/mute/fretted) */
    if (f === -1) {
      ctx.strokeStyle = '#888';
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.moveTo(x - 5, iy - 5); ctx.lineTo(x + 5, iy + 5);
      ctx.moveTo(x + 5, iy - 5); ctx.lineTo(x - 5, iy + 5);
      ctx.stroke();
    } else if (f === 0) {
      ctx.strokeStyle = '#888';
    ...
    /* Finger dot on fretboard */
    if (f > 0) {
      const relF = f - sf;
      if (relF >= 0 && relF < CDP.FRETS) {
        const inBarre = cdpState.barre && cdpState.barre.fret === f
          && s >= cdpState.barre.from && s <= cdpState.barre.to;
        if (!inBarre) {
          const dy = CDP.PAD_TOP + CDP.NUT_H + relF * CDP.CELL_H + CDP.CELL_H * 0.5;
          ctx.fillStyle = '#2a2520';
          ctx.beginPath();
          ctx.arc(x, dy, CDP.DOT_R, 0, Math.PI * 2);
          ctx.fill();
          const fn = cdpState.fingers[s];
          if (fn > 0) {
            ctx.fillStyle = '#fff';
            ctx.font = `bold ${Math.round(CDP.DOT_R * 1.5)}px sans-serif`;
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText(String(fn), x, dy);
          }
        }
      }
    }
...
/* ── SVG diagram renderer (called inside renderScore) ── */
function drawChordDiagramSVG(svg, xCenter, yBottom, diagram) {
  const STR_W = 8, FRET_H = 8, FRETS = 4, NUT_H = 3, DOT_R = 3;
  const IND_H = 9, NAME_H = 13;
  const strCount = diagram.instrument === 'ukulele' ? 4 : 6;
  const diagramW = (strCount - 1) * STR_W;
  const totalH   = NAME_H + IND_H + NUT_H + FRETS * FRET_H;
  const xLeft    = xCenter - diagramW / 2;
  const yName    = yBottom - totalH;
  const yInd     = yName + NAME_H;
  const yNut     = yInd + IND_H;
  const yGridEnd = yNut + NUT_H + FRETS * FRET_H;
  const sf       = diagram.startFret || 1;
  ...
  const g = svgEl('g', { class: 'chord-diag-svg' });

  /* Chord name */
  const nameEl = svgEl('text', {
    x: xCenter, y: yName + 11,
    'text-anchor': 'middle',
    'font-family': "'DM Sans', sans-serif",
    'font-size': '12', 'font-weight': '700', fill: '#2a2520',
  });
  ...
  if (sf === 1) {
    g.appendChild(svgEl('rect', {
      x: xLeft - 1, y: yNut, width: diagramW + 2, height: NUT_H, fill: '#333',
    }));
  } else {
    g.appendChild(svgEl('line', {
      x1: xLeft, y1: yNut, x2: xLeft + diagramW, y2: yNut,
      stroke: '#999', 'stroke-width': '0.8',
    }));
    const frLabel = svgEl('text', {
      x: xLeft - 3, y: yNut + FRET_H * 0.7,
      'text-anchor': 'end', 'font-size': '7', fill: '#666',
    });
    frLabel.textContent = sf + 'fr';
    g.appendChild(frLabel);
  }

  /* Fret lines */
  for (let f = 0; f <= FRETS; f++) {
    g.appendChild(svgEl('line', {
      x1: xLeft, y1: yNut + NUT_H + f * FRET_H,
      x2: xLeft + diagramW, y2: yNut + NUT_H + f * FRET_H,
  ...
  /* String lines */
  for (let s = 0; s < strCount; s++) {
    const sx = xLeft + s * STR_W;
    g.appendChild(svgEl('line', {
      x1: sx, y1: yNut, x2: sx, y2: yGridEnd,
  ...
  if (diagram.barre) {
    ...
    if (relF >= 0 && relF < FRETS) {
      const barY = yNut + NUT_H + relF * FRET_H + FRET_H * 0.5;
      const bx1  = xLeft + from * STR_W - DOT_R * 0.5;
      const bw   = (to - from) * STR_W + DOT_R;
      g.appendChild(svgEl('rect', {
        x: bx1, y: barY - DOT_R, width: bw, height: DOT_R * 2,
        rx: DOT_R, fill: '#222',
      }));
    }
  }
  ...
  for (let s = 0; s < strCount; s++) {
    ...
    if (f === -1) {
      g.appendChild(svgEl('line', { x1: sx-3,y1:iy-3, x2:sx+3,y2:iy+3, stroke:'#666','stroke-width':'1.2' }));
      g.appendChild(svgEl('line', { x1: sx+3,y1:iy-3, x2:sx-3,y2:iy+3, stroke:'#666','stroke-width':'1.2' }));
    } else if (f === 0) {
      g.appendChild(svgEl('circle', { cx:sx, cy:iy, r:3, fill:'none', stroke:'#666','stroke-width':'1.2' }));
    } else {
      const relF = f - sf;
      if (relF >= 0 && relF < FRETS) {
        const inBarre = diagram.barre && diagram.barre.fret === f
          && s >= diagram.barre.from && s <= diagram.barre.to;
        if (!inBarre) {
          const dotY = yNut + NUT_H + relF * FRET_H + FRET_H * 0.5;
          g.appendChild(svgEl('circle', { cx: sx, cy: dotY, r: DOT_R, fill: '#222' }));
        }
      }
    }
  }
...
/* ── Panel helper: update chord name label ── */
function cdpUpdateName() {
  const el = document.getElementById('cdp-chord-name');
  if (el) el.textContent = cdpDetectChordName();
}

function cdpUpdateFretLabel() {
  const el = document.getElementById('cdp-fret-pos');
  if (el) el.textContent = cdpState.startFret + 'fr';
}

/* ── Build preset chord buttons ── */
function cdpBuildPresets() {
  const container = document.getElementById('cdp-presets');
  if (!container) return;
  const presets  = cdpState.inst === 'guitar' ? GUITAR_PRESETS : UKE_PRESETS;
  const voicings = cdpState.inst === 'guitar' ? GUITAR_VOICINGS : UKE_VOICINGS;
  container.innerHTML = '';
  presets.forEach(name => {
    const v = voicings[name];
    if (!v) return;
    const btn = document.createElement('button');
    btn.className = 'cdp-preset-btn';
    btn.textContent = name;
    btn.addEventListener('click', () => {
      const sc = cdpState.inst === 'guitar' ? 6 : 4;
      cdpState.startFret = v.sf || 1;
      cdpState.frets   = [...v.f.slice(0, sc)];
      cdpState.fingers = v.g ? [...v.g.slice(0, sc)] : Array(sc).fill(0);
      cdpState.barre   = v.b ? { ...v.b } : null;
      renderDiagramCanvas();
      cdpUpdateName();
      cdpUpdateFretLabel();
    });
...
/* ── Switch instrument (guitar ↔ ukulele) ── */
function cdpSwitchInstrument(inst) {
  cdpState.inst = inst;
  const sc = inst === 'guitar' ? 6 : 4;
  cdpState.startFret = 1;
  cdpState.frets   = Array(sc).fill(-1);
  cdpState.fingers = Array(sc).fill(0);
  cdpState.barre   = null;
  document.getElementById('cdp-inst-guitar')?.classList.toggle('active', inst === 'guitar');
  document.getElementById('cdp-inst-ukulele')?.classList.toggle('active', inst === 'ukulele');
  renderDiagramCanvas();
  cdpUpdateName();
  cdpUpdateFretLabel();
  cdpBuildPresets();
...
/* ── Canvas cell hit test ── */
function cdpGetCell(e) {
  const canvas = document.getElementById('cdp-canvas');
  const rect   = canvas.getBoundingClientRect();
  const mx = e.clientX - rect.left;
  const my = e.clientY - rect.top;
  const strCount = cdpState.inst === 'guitar' ? 6 : 4;
  const rawS = Math.round((mx - CDP.PAD_L) / CDP.CELL_W);
  const s = Math.max(0, Math.min(strCount - 1, rawS));
  if (my < CDP.PAD_TOP) return { str: s, relFret: -1, isIndicator: true };
  const gridY = my - CDP.PAD_TOP - CDP.NUT_H;
  const relF  = Math.max(0, Math.min(CDP.FRETS - 1, Math.floor(gridY / CDP.CELL_H)));
  return { str: s, relFret: relF, isIndicator: false };
}

/* ── Initialize canvas mouse events ── */
function cdpInitCanvas() {
  const canvas = document.getElementById('cdp-canvas');
  if (!canvas) return;

  canvas.addEventListener('mousedown', (e) => {
    e.preventDefault();
    const { str, relFret, isIndicator } = cdpGetCell(e);
    if (!isIndicator) {
      cdpState._dragStr  = str;
      cdpState._dragFret = cdpState.startFret + relFret;
    } else {
      cdpState._dragStr  = -1;
      cdpState._dragFret = -1;
    }
  });
  ...
  canvas.addEventListener('mouseup', (e) => {
    ...
    if (isIndicator || (cdpState._dragStr >= 0 && cdpState._dragFret < 0)) {
      /* Toggle muted ↔ open */
      cdpState.frets[str] = cdpState.frets[str] === -1 ? 0 : -1;
      cdpState._dragStr = cdpState._dragFret = -1;
      renderDiagramCanvas();
      cdpUpdateName();
      return;
    }

    const absFret = cdpState.startFret + relFret;

    if (cdpState._dragStr >= 0 && cdpState._dragFret === absFret && cdpState._dragStr !== str) {
      /* Drag across same fret → barre */
      const from = Math.min(cdpState._dragStr, str);
      const to   = Math.max(cdpState._dragStr, str);
      cdpState.barre = { fret: absFret, from, to };
      for (let ss = from; ss <= to; ss++) cdpState.frets[ss] = absFret;
    } else if (cdpState._dragStr === str || cdpState._dragStr < 0) {
      /* Click: toggle finger */
      if (cdpState.frets[str] === absFret) {
        cdpState.frets[str] = 0;
        if (cdpState.barre && cdpState.barre.fret === absFret
          && str >= cdpState.barre.from && str <= cdpState.barre.to) {
          cdpState.barre = null;
        }
      } else {
        cdpState.frets[str] = absFret;
      }
    }
...
/* ── Open panel for a given note index ── */
function cdpOpenPanel(noteIndex) {
  const note = appState.notes[noteIndex];
  if (!note) return;
  appState.chordDiagramEditIndex = noteIndex;

  if (note.chordDiagram) {
    const d = note.chordDiagram;
    cdpState.inst      = d.instrument || 'guitar';
    cdpState.startFret = d.startFret  || 1;
    cdpState.frets     = [...d.frets];
    cdpState.fingers   = d.fingers ? [...d.fingers] : Array(d.frets.length).fill(0);
    cdpState.barre     = d.barre ? { ...d.barre } : null;
  } else {
    const sc = cdpState.inst === 'guitar' ? 6 : 4;
    cdpState.startFret = 1;
    cdpState.frets     = Array(sc).fill(-1);
  ...
  document.getElementById('cdp-inst-guitar')?.classList.toggle('active', cdpState.inst === 'guitar');
  document.getElementById('cdp-inst-ukulele')?.classList.toggle('active', cdpState.inst === 'ukulele');
...
/* ── Commit current diagram to note ── */
function cdpCommit() {
  const ci = appState.chordDiagramEditIndex;
  if (ci < 0 || ci >= appState.notes.length) return;
  const sc = cdpState.inst === 'guitar' ? 6 : 4;
  const hasAny = cdpState.frets.slice(0, sc).some(f => f >= 0);
  if (hasAny) {
    appState.notes[ci].chordDiagram = {
      instrument : cdpState.inst,
      startFret  : cdpState.startFret,
      frets      : cdpState.frets.slice(0, sc),
      fingers    : cdpState.fingers.slice(0, sc),
      barre      : cdpState.barre ? { ...cdpState.barre } : null,
      name       : cdpDetectChordName(),
    };
  }
  renderScore();
}

/* ── Delete diagram from current note ── */
function cdpDelete() {
  const ci = appState.chordDiagramEditIndex;
  if (ci < 0 || ci >= appState.notes.length) return;
  delete appState.notes[ci].chordDiagram;
  const sc = cdpState.inst === 'guitar' ? 6 : 4;
  cdpState.startFret = 1;
  cdpState.frets     = Array(sc).fill(-1);
  cdpState.fingers   = Array(sc).fill(0);
...
/* ── Navigate between notes while panel is open ── */
function cdpAdvance(delta) {
  cdpCommit();
  const next = Math.max(0, Math.min(appState.notes.length - 1, appState.chordDiagramEditIndex + delta));
  appState.cursorIndex = next;
  cdpOpenPanel(next);
  renderScore();
}

/* ── Toggle chord diagram mode ── */
function toggleChordDiagramMode() {
  if (appState.lyricMode) toggleLyricMode();
  if (appState.chordMode) toggleChordMode();
  if (appState.measureGapMode) exitGapMode();

  appState.chordDiagramMode = !appState.chordDiagramMode;
  document.getElementById('btn-chord-diagram')?.classList.toggle('active', appState.chordDiagramMode);

  if (appState.chordDiagramMode) {
    if (appState.notes.length === 0) {
      appState.chordDiagramMode = false;
      document.getElementById('btn-chord-diagram')?.classList.remove('active');
      return;
    }
    if (appState.cursorIndex >= appState.notes.length)
      appState.cursorIndex = appState.notes.length - 1;
    hideGhost();
    renderScore();
    cdpOpenPanel(appState.cursorIndex);
  } else {
    closeChordDiagramPanel();
  }
  updateStatusBar();
}

function closeChordDiagramPanel() {
  appState.chordDiagramMode      = false;
  appState.chordDiagramEditIndex = -1;
  document.getElementById('btn-chord-diagram')?.classList.remove('active');
  const panel = document.getElementById('chord-diagram-panel');
...
document.getElementById('btn-chord-diagram')?.addEventListener('click', () => {
  if (!requireFullToolsAccess()) return;
  toggleChordDiagramMode();
});
document.getElementById('cdp-close')?.addEventListener('click', closeChordDiagramPanel);
document.getElementById('cdp-inst-guitar')?.addEventListener('click', () => cdpSwitchInstrument('guitar'));
document.getElementById('cdp-inst-ukulele')?.addEventListener('click', () => cdpSwitchInstrument('ukulele'));
document.getElementById('cdp-fret-down')?.addEventListener('click', () => {
  cdpState.startFret = Math.max(1, cdpState.startFret - 1);
  renderDiagramCanvas(); cdpUpdateFretLabel();
});
document.getElementById('cdp-fret-up')?.addEventListener('click', () => {
  cdpState.startFret = Math.min(20, cdpState.startFret + 1);
  renderDiagramCanvas(); cdpUpdateFretLabel();
});
document.getElementById('cdp-clear-fingers')?.addEventListener('click', () => {
  const sc = cdpState.inst === 'guitar' ? 6 : 4;
  cdpState.startFret = 1; cdpState.barre = null;
  cdpState.frets   = Array(sc).fill(-1);
  cdpState.fingers = Array(sc).fill(0);
  renderDiagramCanvas(); cdpUpdateName(); cdpUpdateFretLabel();
});
document.getElementById('cdp-insert')?.addEventListener('click', cdpCommit);
document.getElementById('cdp-delete')?.addEventListener('click', cdpDelete);
document.getElementById('cdp-prev')?.addEventListener('click', () => cdpAdvance(-1));
document.getElementById('cdp-next')?.addEventListener('click', () => cdpAdvance(1));
...
dom.canvas.addEventListener('mousedown', (e) => {
  ...
  if (hit) {
    ...
    const isNoteClickMode = appState.dynamicsMode || appState.articulationMode ||
      appState.chordMode || appState.lyricMode || appState.chordDiagramMode ||
...
dom.canvas.addEventListener('mousemove', (e) => {
  ...
  /* ★ show ghost note preview (not in chord/lyric/diagram mode) */
  if (appState.chordMode || appState.lyricMode || appState.chordDiagramMode) { hideGhost(); return; }
  updateGhost(mx, my);
});
...
function handleSingleClick(mx, my) {
  ...
  if (hit) {
    ...
    /* In chord diagram mode, clicking a note opens the CDP panel (voice 1 only) */
    if (appState.chordDiagramMode) {
      renderScore();
      cdpOpenPanel(appState.cursorIndex);
      return;
  ...
  /* Don't place new notes while in chord/lyric/diagram mode */
  if (appState.chordMode || appState.lyricMode || appState.chordDiagramMode) return;
...
document.addEventListener('keydown', (e) => {
  ...
  /* ── Escape exits dynamics / articulation / repeat / song form / gap / diagram mode ── */
  if (key === 'Escape') {
    if (appState.chordDiagramMode){ e.preventDefault(); closeChordDiagramPanel(); return; }
...
dom.btnUndo.addEventListener('click', () => {
  ...
  if (appState.chordDiagramMode && appState.notes.length === 0) closeChordDiagramPanel();
  renderScore();
});
dom.btnClear.addEventListener('click', () => {
  ...
  if (appState.chordDiagramMode) closeChordDiagramPanel();
  if (appState.repeatMode) exitRepeatMode();
  renderScore();
});
...
function updateStatusBar() {
  ...
  /* chord status */
  if (appState.chordDiagramMode) {
    dom.statusChord.textContent = '⊞ Diagram Mode';

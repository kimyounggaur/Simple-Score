...
function normalizeUserProfile(profile = {}) {
  return {
    ...
    organization: normalizeOptionalText(profile.organization, 120),
    usagePurpose: normalizeOptionalText(profile.usagePurpose, 80),
    referralSource: normalizeOptionalText(profile.referralSource, 80),
...
function normalizeUserRecord(user = {}) {
  ...
  const profile = normalizeUserProfile({
    ...
    usagePurpose: user.profile?.usagePurpose ?? user.usagePurpose,
    referralSource: user.profile?.referralSource ?? user.referralSource,
    marketingConsent:
      user.profile?.marketingConsent ?? user.marketingConsent ?? false,
  });
...
export async function findUsersByName(name) {
  ...
  return db.users
    .filter((user) => String(user.name || "").trim().toLowerCase() === normalizedName)
...
export async function updateUserPassword(id, { passwordHash, passwordSalt }) {
  let updatedUser = null;
